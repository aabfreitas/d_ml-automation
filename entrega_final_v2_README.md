...
| Elemento | O que faz |
|---|---|
| **Switch** | IP, porta, transporte (SSH/Telnet), usuário, senha e senha de enable. Desativado no modo simulado |
| **Transporte** | SSH por padrão; Telnet como alternativa para switches com SSH legado/instável (a porta muda de 22 para 23 automaticamente) |
...
| **Backups** | Arquivos gerados, com link para download e um botão **Restaurar** em cada um (rollback) |
| **Logs de execução** | Histórico de todas as execuções (aplicar/validar/restaurar), persistente entre sessões |
...
# apagar o estado do dispositivo simulado, voltando-o à fábrica
python cli.py --simular --resetar-simulado
```
...
31 testes: 14 cobrindo o switch (normalização de nomes, validação de entrada, parsing das
saídas do IOS, fluxo completo), 6 cobrindo o roteador (parsing de subinterfaces,
validação de IP/máscara, fluxo completo), e 11 cobrindo rollback, persistência do
dispositivo simulado, convenção de nomenclatura e o log de execuções.
...
**Telnet como transporte alternativo, e `netmiko[par4]` fixando a versão do Paramiko.**
Contra o laboratório real, um switch respondia SSH com algoritmos antigos
(`diffie-hellman-group1-sha1`, cifras CBC) que o Paramiko 5.0+ removeu do código por
completo — não é uma questão de configuração, o suporte simplesmente não existe mais
nessa versão. `requirements.txt` fixa `netmiko[par4]`, o extra oficial do próprio Netmiko
que resolve isso (`paramiko>=4.0,<5.0`, a última série que ainda suporta esses algoritmos,
aliás habilitados por padrão). Separadamente, `--transporte telnet` existe como alternativa
para quando o SSH de um equipamento estiver genuinamente instável (não só com algoritmo
antigo, mas com a conexão caindo) — investigação completa em
