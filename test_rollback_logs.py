...
def test_nome_de_vlan_conforme_nao_gera_aviso_de_convencao():
    from src.convencao import aviso_convencao_nome_vlan

    assert aviso_convencao_nome_vlan("VLAN_DADOS") is None
    assert aviso_convencao_nome_vlan("VLAN_VOZ") is None
    assert aviso_convencao_nome_vlan("VLAN_SEGURANCA") is None


def test_nome_de_vlan_fora_da_convencao_gera_aviso():
    from src.convencao import aviso_convencao_nome_vlan

    aviso = aviso_convencao_nome_vlan("isolada", 99, rotulo="VLAN")
    assert aviso is not None
    assert "convenção" in aviso
    assert "99" in aviso


def test_switch_com_vlan_fora_da_convencao_gera_aviso_mesmo_pedida():
    # Reproduz o cenário reportado: a VLAN 99 foi pedida explicitamente (não é
    # "fora do padrão" por não ter sido pedida), mas o NOME dela não segue a
    # convenção VLAN_<algo> — isso precisa gerar um aviso à parte.
    desejado = DesiredState.from_payload(
        {
            "hostname": "CD-SP1-SW001",
            "vlans": [
    ...
    avisos_de_nome = [a for a in desejado.avisos if "isolada" in a]
    assert len(avisos_de_nome) == 1
    assert "99" in avisos_de_nome[0]


def test_router_subinterface_sem_descricao_gera_aviso():
    desejado = RouterDesiredState.from_payload(
        {"hostname": "CD-SP1-CR001", "subinterfaces": [{"vlan_id": 10, "ip": "192.168.10.1"}]}
    )
    avisos_de_descricao = [a for a in desejado.avisos if "sem descrição" in a]
    assert len(avisos_de_descricao) == 1
    assert ".10" in avisos_de_descricao[0]


def test_router_subinterface_com_descricao_fora_da_convencao_gera_aviso():
    desejado = RouterDesiredState.from_payload(
        {
            "hostname": "CD-SP1-CR001",
            "subinterfaces": [{"vlan_id": 99, "ip": "192.168.99.1", "descricao": "isolada"}],
        }
    )
    avisos_de_nome = [a for a in desejado.avisos if "isolada" in a]
    assert len(avisos_de_nome) == 1
